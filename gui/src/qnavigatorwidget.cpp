#include "gui/qnavigatorwidget.h"
#include <stdio.h>
#include <Qt>
using namespace GMapping;


QNavigatorWidget::QNavigatorWidget( QWidget * parent, const char * name, Qt::WFlags f)
...
void QNavigatorWidget::mousePressEvent ( QMouseEvent * e ){
	...
	int mx=p.x();
	int my=height()-p.y();
	if (!(e->buttons()&(Qt::ShiftModifier | Qt::LeftButton))) {
		if (trajectorySent)
	...
	if (e->buttons()&(Qt::ControlModifier | Qt::RightButton)){
		e->accept();
...
void QNavigatorWidget::paintEvent ( QPaintEvent * ){
	...
	//QImage::bitBlt(this,0,0,&pixmap,0,0,pixmap.width(),pixmap.height(),Qt::CopyROP);
	QPainter *p = (QPainter*)this;
	p->drawPixmap(QPoint(0, 0), pixmap, QRect(0, 0, pixmap.width(),pixmap.height()));
}
